...
class Pizza(GraphicsEntity):  
    ...
    def render(self,screen):
        ...
        screen.blit(img,(self.pose[0]/7.5*250-size,500-self.pose[1]/7.5*250-size))
...
class Scanner():
    ...
    def render(self,screen,pose):
        vertices = [(pose[0]/7.5*250,500-pose[1]/7.5*250)]
        for i in range(int(math.degrees(-self.range / 2)), int(math.degrees(self.range / 2)) + 1):
            rads = math.radians(i)+pose[2]
            vertices.append(((pose[0] + self.radius * math.cos(rads))/7.5*250,500-(pose[1] + self.radius * math.sin(rads))/7.5*250))
        vertices.append((pose[0]/7.5*250,500-pose[1]/7.5*250))
