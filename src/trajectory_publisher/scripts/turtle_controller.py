...
CHECKPOINT_PATH = os.path.join(
    _SCRIPTS_DIR,
    "logs/pose_trajectory_3DV2/cfm/H64_T100/20260505-1157/state_192000.pt",
)
TURTLESIM_ORIGIN_X = 5.544       # TurtleSim default spawn x (world meters)
TURTLESIM_ORIGIN_Y = 5.544       # TurtleSim default spawn y (world meters)
...
class TurtleController(Node):
    ...
    def _traj_context_cb(self, msg):
        # LLM tool docstring: "Assume the robot starts at (0,0,0) ... pointing toward goal."
        # We treat s_goal as a displacement from the turtle's current pose at arrival
        # (world-axis-aligned meters). If pose is not yet known, fall back to spawn origin.
        if self._current_pose is not None:
            origin_x = self._current_pose.x
            origin_y = self._current_pose.y
        else:
            origin_x = TURTLESIM_ORIGIN_X
            origin_y = TURTLESIM_ORIGIN_Y
        goal_x_world = float(msg.s_goal.x) + origin_x
        goal_y_world = float(msg.s_goal.y) + origin_y
        ...
        self.get_logger().info(
            ...
            f'goal=({goal_x_world:.3f}, {goal_y_world:.3f}) world'
        )
    ...
    def _init_pid(self):
        ...
        self._active_start    = (TURTLESIM_ORIGIN_X, TURTLESIM_ORIGIN_Y, 0.0)
    ...
    def _goal_cb(self, msg: Point):
        ...
        self._phase = self._ACTIVE
        self.get_logger().info(
            f'[PID] Manual goal: ({self.goal_x:.3f}, {self.goal_y:.3f}) — CFM resampled'
        )
    ...
    def _resample_trajectory(self, pose, label: str = 'sample'):
        ...
        # Convert to world frame for execution
        waypoints_world = waypoints.copy()
        waypoints_world[:, 0] += TURTLESIM_ORIGIN_X
        waypoints_world[:, 1] += TURTLESIM_ORIGIN_Y
    ...
    def _save_pid_log(self, cfm_waypoints_world: np.ndarray):
        ...
        try:
            ...
            np.savez(
                path,
                cfm_waypoints=cfm_waypoints_world.astype(np.float32),
